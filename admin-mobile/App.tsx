...
import { RiderAppScreen } from './src/screens/RiderAppScreen';
...
function DrawerContent(props: DrawerContentComponentProps) {
  const { admin } = useApp(); const signOut = async () => { await supabase.auth.signOut(); props.navigation.closeDrawer(); props.navigation.getParent()?.getParent()?.navigate('Login' as never); };
...
function StaffGate(props: any) { const { session, admin } = useApp(); return session && admin ? (admin.role === 'Rider' ? <RiderAppScreen /> : <AdminNavigator />) : <LoginScreen {...props} />; }
...
function RootApp() {
  ...
  useEffect(() => { if (session && admin && admin.role !== 'Rider') refresh().catch(() => {}); }, [session, admin, refresh]);
  ...
  return <NavigationContainer theme={{ ...DefaultTheme, colors: { ...DefaultTheme.colors, background: colors.background, primary: colors.primary } }}><Root.Navigator initialRouteName={session && admin ? 'Admin' : 'Login'} screenOptions={{ headerShown: false, animation: 'fade' }}><Root.Screen name="Login" component={LoginScreen} /><Root.Screen name="Admin" component={StaffGate} /></Root.Navigator></NavigationContainer>;
}
